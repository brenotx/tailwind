...
export interface TreeProps<T>
  extends Omit<AriaTreeGridListProps<T>, "children">,
    CollectionProps<T>,
...
function TreeInner<T extends object>({
  ...
  return (
    <FocusScope>
      ...
      >
        <Provider values={[[TreeStateContext, state]]}>{children}</Provider>
        {emptyState}
      </div>
    </FocusScope>
  );
...
function TreeRow<T>({ item }: { item: Node<T> }) {
  ...
  let { rowProps, gridCellProps, ...states } = useTreeGridListItem(
    { node: item },
    state,
    ref
  );
  ...
  useEffect(() => {
    if (hasChildRows && !expandButtonRef.current) {
      console.warn(
        "Expandable tree rows must contain a expand button so screen reader users can expand/collapse the row."
      );
    }
...
function flattenTree<T>(
  ...
  let visitNode = (node: Node<T>) => {
    if (node.type === "item") {
      ...
      if (
        node.level === 0 ||
        (expandedKeys === "all" && node.type === "item") ||
        (expandedKeys !== "all" &&
          parentKey != null &&
